...
class ShearInverseDemoExample(Scene):
    """
    The inverse of a rightward shear [[1, 1], [0, 1]] is a leftward
    shear [[1, -1], [0, 1]].
    """

    def construct(self):
        title = Tex(r"Inverse of rightward shear $=$ leftward shear",
        ...
        S_right = np.array([[1.0, 1.0], [0.0, 1.0]])
        S_left = np.array([[1.0, -1.0], [0.0, 1.0]])
        ...
        def M_of():
            ...
            if s <= 1:
                return (1 - s) * np.eye(2) + s * S_right
            alpha = s - 1
            return (1 - alpha) * S_right + alpha * (S_left @ S_right)
        ...
        def stage_str():
            ...
            if s < 1.05: return r"rightward shear"
            return r"leftward shear inverse: identity restored"
        ...
        info = VGroup(
            Tex(r"$S=\begin{pmatrix}1&1\\0&1\end{pmatrix}$",
                color=BLUE, font_size=22),
            Tex(r"$S^{-1}=\begin{pmatrix}1&-1\\0&1\end{pmatrix}$",
